...
const MODULE_FIELDS = {
  projects:"id,title,client,status,pct,description,started_at,due_at,budget,paid,deliverables,tags,client_id,owner_type,owner,context,template,category,responsible,archive,payments,checklist,history",
...
function dbAlmaToState(row, m){
  return {
    ...
    projects:   (m.projects   || []).map(x => ({ _id:x.id, t:x.title, st:x.status, pct:x.pct, client:x.client, desc:x.description, start:x.started_at, due:x.due_at, budget:x.budget, paid:x.paid, deliverables:x.deliverables, tags:x.tags, client_id:x.client_id, owner_type:x.owner_type, owner:x.owner, context:x.context, template:x.template, category:x.category, responsible:x.responsible, archive:x.archive, abonos:Array.isArray(x.payments)?x.payments:[], checklist:Array.isArray(x.checklist)?x.checklist:[], hist:Array.isArray(x.history)?x.history:[] })),
